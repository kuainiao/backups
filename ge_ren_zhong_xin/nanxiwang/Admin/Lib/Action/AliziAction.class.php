...
class AliziAction extends Action {
    ...
    function proccess($module='') 
    {
        ...
        if($_REQUEST['do']==='delete'){
            ...
            {
                $CategoryInfo['name'] = $_REQUEST['name'];
                $CategoryInfo['sort_order'] = $_REQUEST['sort_order'];
                $CategoryInfo['type'] = 1;

                $category = D('category');
                $res1 = $category->where($CategoryInfo)->find();
                if(!$res1)
                {
                    $res = $category->data($CategoryInfo)->add();
                    if(!$res){
                        $this->ajaxReturn(null, '添加分类出错', 0);
                    }
                    else
                    {
                        $this->ajaxReturn(null, '添加分类成功', 1);
                    }
                }
                else
                {
                    $this->ajaxReturn(null, '分类已存在', 0); 
                }
            }
            // 添加商品
            else
