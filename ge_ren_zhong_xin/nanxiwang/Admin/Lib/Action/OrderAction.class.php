...
class OrderAction extends AliziAction {
    ...
    function index($payment=0)
    {
        ...
        if(isset($_GET['aliziExcel']))
        {
            ...
            { 
                $payment[$key] = $value['name'];
            }
            $jsonPayment = json_encode($payment);
            foreach($list as &$li)
            {
                // 去除名称显示前#前的内容
                $ItemParamsTemp = explode('#', $li['item_params']);
                $li['item_params'] = $ItemParamsTemp[0];
                // 套餐选项
                $TempExtends = explode(',', $li['item_extends']);
                $temp1 = '';
                foreach($TempExtends as $v)
                {
                    if(strstr($v, '\u65e0'))
                    {
                        unset($v);
                ...
                // 将信息拼接成unicode
                $info = implode(',', $temp1);
                if(strstr($info, '}') && !strstr($info, '{'))
                {
                    $li['item_extends'] = '{' . $info;
                }
                else if(strstr($info, '{') && !strstr($info, '}'))
                {
                    $li['item_extends'] = $info . '}';
                }
                else if(!strstr($info, '{') && !strstr($info, '}'))
        ...
        foreach($list as $k => $v)
        {
            // var_dump($v['item_params']);
            // 去除商品名称里面#号前内容
            $TempItemParams = explode('#', $v['item_params']);
            $TempList[$k] = $v;
            $TempList[$k]['item_params'] = $TempItemParams[0];
        }
        $TempList2 = '';
        $TempExtends2 = '';
        foreach($TempList as $key => $v)
        {
            // 去除item_extends #号前内容
            $TempExtends1 = explode(',', $v['item_extends']);
            foreach($TempExtends1 as $val)
            {
                $TempArr = explode('#', $val);
                $TempExtends2[] = '"'.$TempArr[1];
            }
            $TempList2[$key] = $v;
            $TempList2[$key]['item_extends'] = '{'.implode(',', $TempExtends2);
        }
        $list = $TempList2;
    ...
    function view($id='')
    {
        ...
        // 去除名称显示前#前的内容
        $ItemParamsTemp = explode('#', $info['item_params']);
        $info['item_params'] = $ItemParamsTemp[0];

        // 套餐选项
        // 去除item_extends #号前内容
        $TempExtends = explode(',', $info['item_extends']);
        ...
        // 将信息拼接成unicode
        $infos = implode(',', $temp1);
        if(strstr($infos, '}') && !strstr($infos, '{'))
        {
            $info['item_extends'] = '{' . $infos;
        }
        else if(strstr($infos, '{') && !strstr($infos, '}'))
        {
            $info['item_extends'] = $infos . '}';
        }
        else if(!strstr($infos, '{') && !strstr($infos, '}'))
        ...
        $info['item_extends'] = '{'.implode(',', $temp1);
