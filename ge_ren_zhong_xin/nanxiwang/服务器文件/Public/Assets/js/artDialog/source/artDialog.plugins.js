 ...
 * @param cache	{function}			是否允许缓存. 默认true
 */
$.open = $.dialog.open = function(url, options){
	var opt = $.extend({id:'Open',fixed: true,padding:0}, options); 
	opt.content='<iframe src="'+url+'" name="Opendefault" frameborder="0" allowtransparency="true" scrolling="false" style="width:'+opt.width+'px; height:'+opt.height+'px;border: 0px none;"></iframe>';
	return $.dialog(opt);
};
 ...
 * @param   {int}                   图标
 * @param   {Function}              (可选) 回调函数，如果等于false，则确定关闭后刷新当前页
 */
$.alert = $.dialog.alert = function (content, icon, callback) {
	if(callback===false && icon==1)callback = function(){window.location.reload()};
	if(!isNaN(icon)){
		content = '<table><tr><td class="icon icon-'+icon+'"></td><td style="font-size:14px;">'+content+'</td></tr></table>';
	}
    return $.dialog({
        id: 'Alert',
        fixed: true,
...
$.confirm = $.dialog.confirm = function (content, ok, cancel) {
	content = '<table><tr><td class="icon icon-3"></td><td style="font-size:14px;">'+content+'</td></tr></table>';
    return $.dialog({
        id: 'Confirm',
        fixed: true,
        lock: true,
