    ...
    <div class="innner order_div success">
        <div class="order" style="min-height: calc(100vh - 244px);">
            <ul>
				...
				<?php if(!empty($order["item_params"])): ?><li><label><?php echo lang('itemPackage_colon');?></label><span><?php echo ($order["item_params"]); ?></span></li><?php endif; ?>
				<?php $extends = json_decode($order['item_extends'],true); ?>
				<?php if(!empty($extends)): if(is_array($extends)): $i = 0; $__LIST__ = $extends;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><label><?php echo ($key); echo lang('colon');?></label><span><?php echo ($vo); ?></span></li><?php endforeach; endif; else: echo "" ;endif; endif; ?>
