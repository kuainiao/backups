...
function isMobileNum($mobile) {
    ...
    switch(C('DEFAULT_LANG')){
        case 'zh-cn': return preg_match("/1[345789]{1}\d{9}$/", $mobile);break;
...
// function getCaches($where,$refresh=false,$time=864000)
// {
//    // 1 // var_dump($where);exit();
//     // $what['g_id'] = $where['g_id'];
//     // var_dump($what);exit();
//     // $Goods = M('Goods');
//     // $what['b_s'] = $Goods->where($where)->getField('b_s');
//     // if(empty($what['b_s'])){
//     //     $this->error('没有找到什么');
//     //     exit();
//     // }
...
//    // 2 $cacheName = 'Goods'.implode('', $where);
//    //  $data = cache($cacheName);
//    // 1 // var_dump($data);
//     // 2if(empty($data) || $refresh==true){
//        // 1 // echo 1;
//         // $table1 = C("DB_PREFIX").'goods';
//         // $table2 = C("DB_PREFIX").'item';
//         // 链表查询
//         // $sql = 'SELECT * FROM '.$table1.','.$table2.' WHERE '.$table1.'.g_id = '.$table2.'.g_id AND '.$table1.'.b_s = '.$table2.'.b_s AND '.$table1.'.g_id = '."'$GId'".' AND '.$table1.'.sta = 1 ';
        
//         // $res = $Goods->query($sql);
//         // $what['status'] = 1;
//         // 2$Item = M('Item');
//         // $data = $Item->where($where)->find();
// // 1// var_dump($data);exit();
//         // $data = M($Model)->where($where)->find();
//         // 2cache($cacheName,$data,$time);
//     // 2}
//     // 2return $data; 
// }
