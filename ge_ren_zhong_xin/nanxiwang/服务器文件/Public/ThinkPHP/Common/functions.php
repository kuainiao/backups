...
function U($url='',$vars='',$suffix=true,$redirect=false,$domain=false) {
    ...
    if(C('ALIZI_ROUTE')==true && C('URL_MODEL')==2){
         $uri = ALIZI_URL($url,$vars);
         if($uri){
			 return $uid?$uri.'?uid='.$uid:$uri;
		 }
    }
...
function ALIZI_URL($url,$vars){
    $rules = C('ROUTE_RULES');
	$uri = '';
    list($module,$action) = explode('/', $url);
    if(isset($rules[$module]) && isset($rules[$module]['a']) && empty($vars)){
        $action = str_replace($module.'/', '', $url);
        $uri = C('ALIZI_ROOT').($url=='/'?'':str_replace(':a', $action,$rules[$module]['a']));
    }elseif(!empty($vars) && isset($rules[$module][$action])){
        $key = implode(':',array_keys($vars));
        $uri = $rules[$module][$action][$key];
        foreach($vars as $k=>$v){
            $uri = str_replace(':'.$k, $v, $uri);
        }
        $uri = C('ALIZI_ROOT').$uri;
    }
    return $uri;
}
/**
