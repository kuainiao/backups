...
class ApiAction extends Action{
    ...
    public function aliziBooking(array $data)
    {
        ...
        if($item_params){
            $paramsArray = array();
            foreach($item_params as $params){ $paramsArray[] = $params['title']; }
            //return array('status'=>0,'message'=>implode('|',$paramsArray));
            if(!in_array($data['item_params'],$paramsArray)) return array('status'=>0,'message'=>lang('pleaseSelect_package'));
        }else{
             $data['item_params'] = '';
        }
        ...
        if($alizi=$Order->create($data)){
            ...
            return array('status'=>0,'message'=>lang('111111111'));
        }    
    }
    ...
    public function getAliziPayment($sn,$payment_id=''){
        ...
        $paymentInfo=array(
            ...
            7=>array('info'=> preg_replace('/\r\n/', '',nl2br($this->aliziConfig['yunpay_info'])),'math'=>'*'.$this->aliziConfig['yunpay_discount'],),
        );
        ...
        if($this->aliziConfig['payment_global']==1){
            ...
            if(empty($this->aliziConfig['yunpay_status'])) unset($payment[7]);
    ...
    public function send($order_id,$status=0,$remark='',$print=false){
        ...
        if(in_array($status, $send_status) || ($status==3 && $item['is_auto_send']==1)){
            ...
            //$content .= "<div style='display:block !important;font-family:MicroSoft Yahei;padding-top:80px;text-align:right;font-size:12px;color:#888;'>&copy; 2015-".date('Y')." <a href='' target='_blank'>楠溪王订单系统</a> All Rights Reserved.</div>";
    ...
    private function sendSMS($order_id){
        ...
        if($sms[$status]['status']==1 && !empty($sms[$status]['content'])){
			...
			//云片网短信
			if(trim($this->aliziConfig['sms_account'])=='yunpian'){
				$url = 'https://sms.yunpian.com/v1/sms/send.json';
				$data = array(
					'apikey'=>$this->aliziConfig['sms_password'],
					'mobile'=>$order['mobile'],
					'text'=>$content,
				);
    ...
    private function aliziCheck(&$data,$extends){
        $page  = trim($data['page']);
        if(in_array($page,array('index','item','wap'))){
	...
	if(isset($data['region'])){
		$data = array_merge($data,$data['region']);
